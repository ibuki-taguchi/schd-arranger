...
router.get("/new", authEnsurer.ensure, csrfProtection, (req, res, next) => {
  res.render("new", { user: req.user, csrfToken: req.csrfToken() });
});
router.post("/", authEnsurer.ensure, csrfProtection, (req, res, next) => {
    ...
    if (schedule) {
      Candidate.findAll({
        ...
        Availability.findAll({
          ...
          // 全ユーザー、全候補で二重ループしてそれぞれの出欠のx値がない場合には、「欠席」を設定する
